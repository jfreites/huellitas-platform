...
export default function Login() {
  ...
  const handleRequestOtp = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (res.success) {
        ...
        setStep(2);
        if (!isEmail) setContactPhone(identifier);
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    if (isEmail && !contactPhone.trim()) {
      setError('Por favor ingresa un número de celular de contacto.');
      return;
    }
  ...
  const handleEmailMagicLink = async () => {
    ...
    if (err) {
      ...
      setMessage('Te enviamos un enlace mágico al correo. Ábrelo para iniciar sesión.');
    }
  };
      ...
      <div className="w-full max-w-md rounded-2xl border border-border bg-card p-6 sm:p-8 text-card-foreground shadow-md">
        <div className="text-center mb-6">
          ...
          <p className="text-xs sm:text-sm text-foreground/60 mt-1">
            Requerimos verificación rápida para asegurar reportes reales de mascotas y evitar fraudes.
          </p>
        </div>
          ...
          <form onSubmit={handleRequestOtp} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <p className="text-[10px] text-foreground/45 mt-1 leading-normal">
                Te enviaremos un código de 6 dígitos (SMS) o un magic link (correo). Si el correo no llega, revisa la carpeta de spam o usa el botón &quot;Enviar enlace mágico&quot;.
              </p>
            </div>
            ...
            >
              {isPending ? 'Enviando código...' : 'Solicitar Código'}
            </button>
          </form>
        )}
        ...
        {step === 2 && (
          <form onSubmit={handleVerifyOtp} className="space-y-4">
            ...
            >
              ¿No recibiste el código? Reenviar enlace mágico al correo
            </button>
          </form>
        )}
